...
public class MetodoPagoRest {
	...
	//GUARDAR METODOPAGO
			@PostMapping ("GuardarMetodo")
			private MetodoPago guardarMetodo(@RequestBody MetodoPago metodo) {
			    try {
			        MetodoPago metodoGuardado = metodoPagoService.saveMetodo(metodo);
			        return metodoGuardado;
			    } catch (Exception e) {
			        e.printStackTrace();
			        throw new RuntimeException("No se pudo guardar el Metodo", e);
			    }
			}
